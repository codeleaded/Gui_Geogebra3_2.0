...
vec3d WorldOrigin = { 0.0f,-4.0f,15.0f,1.0f };
float FunctionYaw = 0.0f;	
float FunctionPitch = 0.0f;
...
void Triangles_Project(){
	...
	for (int i = 0;i<meshCube.tris.size;i++){
		...
		//vec3d_DotProduct(tri.n,vCameraRay) < 0.0f
		if (1){
...
void Setup(AlxWindow* w){
	...
	fYaw = 0.0f;	
	fPitch = 0.0f;
	vCamera = (vec3d){ -5.0f,5.0f,-5.0f,1.0f };

	WorldOrigin = (vec3d){ 0.0f,-5.0f,18.0f,1.0f };
}
...
void Update(AlxWindow* w){
	...
	matView = Matrix_MakeIdentity();
	//matView = Matrix_MakeRotationX(FunctionPitch);
	matView = Matrix_MultiplyMatrix(matView,Matrix_MakeRotationY(FunctionYaw));
	matView = Matrix_MultiplyMatrix(matView,Matrix_MakeTranslation(WorldOrigin.x,WorldOrigin.y,WorldOrigin.z));
...
int main(){
    if(Create("Geogebra 3D 2.0",2500,1200,1,1,Setup,Update,Delete))
        Start();
    return 0;
}